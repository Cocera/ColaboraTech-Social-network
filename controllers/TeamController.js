    ...
    async addMembers(req, res, next) {
        try {
            ...
            if (!project) {
                ...
                const existingMembers = projectTeam
                    .members
                    .filter((member) => members.includes(member.toString()));
    ...
    async removeMembers(req, res, next) {
        try {
            ...
            if (!project) {
                ...
                const existingMembers = projectTeam
                    .members
                    .filter((member) => membersToDelete.includes(member.toString()));

                if (existingMembers.length == 0) {
                    return res
                        .status(400)
                        .send({message: `CAN NOT DELETE: Users with id ${membersToDelete} are not in this team`});
                } else {
                    await Team.findByIdAndUpdate(project.team, {
                        $pull: {
                            members: {
