...
const CommentController = {
	async create(req, res) {
		try {
			...
			res
				.status(201)
				.send({
					message: `Comment created successfully in post with id: ${req.params.post_id}.`,
					bodyText: comment.bodyText,
				});
		} catch (error) {
			console.error(error);
			res.status(500).send({ message: "Error while creating comment.", error });
		}
	},
	...
	async update(req, res) {
		try {
			...
			res
				.status(200)
				.send({
					message: `Comment with id ${req.params.comment_id} updated.`,
					bodyText: commentToUpdate.bodyText,
				});
		} catch (error) {
			console.error(error);
			res.status(500).send({ message: "Error while updating comment.", error });
		}
	},
	...
	async insertLike(req, res) {
		try {
			...
			console.error(error);
			res.status(500).send({ message: "Error while liking comment.", error });
		}
	},
		...
		try {
			...
			if (!likedComment) {
				...
				return res
					.status(400)
					.send({
						message: `${req.user.name} has to like comment before unlike`,
					});
			} else {
				await Comment.findByIdAndUpdate(
					req.params.comment_id,
			...
			res
				.status(500)
				.send({ message: "Error while removing like from comment.", error });
		}
	},
	...
	async delete(req, res) {
		try {
			...
			res.send({
				message: `${req.user.name}'s comment with id ${req.params.comment_id} deleted.`,
			});
		} catch (error) {
			console.error(error);
			res
				.status(500)
				.send({
					message: `Error while deleting comment with id ${req.params.comment_id}.`,
					error,
				});
		}
	},
